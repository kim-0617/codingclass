...
const goodAudio = document.querySelector('#goodAudio');
const badAudio = document.querySelector('#badAudio');
const playBtn = document.querySelector('.search__audio .play');
const stopBtn = document.querySelector('.search__audio .stop');
...
// 전체 속성 갯수
const totalProps = document.querySelector('.search__info .num');

// 맞춘 속성 갯수
const goodProps = document.querySelector('.search__info .now');
...
let gameTime = 120; // 전체 시간
let timeRemain = gameTime;
let score = 0;  // 점수
let answers = {}; // 새로운 정답
let tid = 0; // 클리어 인터벌
let tid2 = 0; // 클리어 타임아웃
...
function updateList() {
    ...
    totalProps.innerText = cssProperty.length;
}
updateList();
...
function checkInput() {
    ...
    if (answers.hasOwnProperty(input) && answers[input] === false) {
        answers[input] = true;
        ...
        // 정답 카운트 올리기
        score++
        goodProps.innerText = score;
    }
}

// 정답 체크하기 : 정답을 객체 파일로 관리
...
function missAnswers() {
    ...
    for (let key in answers) {
        if (answers[key] === false) {
            searchMissAnswers.innerHTML += `<span>${key}</span>`;
        }
    }
}
...
function endQuiz() {
    ...
    searchResult.innerHTML = `게임이 끝났습니다. <br> ${cssProperty.length}중에 ${score}개를 맞추었습니다. <br> 당신의 점수는 ${point}점입니다.`
    ...
    // start 버튼 이벤트 제거
    searchStart.removeEventListener('click', startQuiz);
}
